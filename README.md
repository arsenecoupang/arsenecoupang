# Jihan Lee

**Brief**
* 🏛️ Student at **Korea Digital Media High School** (Dimigo)
* 🇰🇷 Front-end Developer based in South Korea
* ✉️ [your-email@example.com](mailto:suhoon222@outlook.com)

---

**Projects**
* **Personal Portfolio v2** | Front-end Developer (26.03)
  - Framer Motion 기반 인터랙션 및 브라우저 렌더링 최적화
  - Lighthouse 성능 지표 99점 달성
* **Amico** | Front-end Developer (25.12 ~ 26.02)
  - Supabase Realtime 기반 실시간 채팅 서비스 개발
  - GitHub Actions를 활용한 CI/CD 파이프라인 구축
* **REST API Practice** | Front-end Developer (25.12)
  - Axios 기반 CRUD 로직 및 RESTful API 연동 실습

**Activity**
* **LUNA** | 16th Member (26.03 ~ Present)
* **School Coding Club** | Member (24.04 ~ 25.05)
  - Flask 기반 팀 프로젝트 참여 및 Gitflow 브랜치 전략 도입

**Tech Stack**
* **Frontend**: React, TypeScript, Tailwind CSS, Framer Motion, shadcn/ui
* **Tools**: Gitflow, GitHub Actions, Vite, Biome, Vercel, Supabase

---

**Contact**
[![Github](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/arsenecoupang)
[![Instagram](https://www.instagram.com/jih_an7891/)
